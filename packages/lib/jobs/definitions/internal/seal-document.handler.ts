...
const formatRecipientForPdfSignature = (recipient: Pick<Recipient, 'name' | 'email' | 'role' | 'signingStatus'>) => {
  const name = recipient.name?.trim();

  return name ? `${name} <${recipient.email}>` : recipient.email;
};

const getPdfSignatureMetadata = (recipients: Array<Pick<Recipient, 'name' | 'email' | 'role' | 'signingStatus'>>) => {
  ...
  return {
    reason: `Signed in Documenso by: ${signers.join(signerSeparator)}`,
    contactInfo: signerEmails.join(signerSeparator),
  };
};
